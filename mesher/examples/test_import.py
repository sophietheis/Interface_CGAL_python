...
from mesher.cgal_mesher import Point_3, Mesh
...
def draw_ugly(list_intersected_faces, vertices, faces, ax):
    for i in range(0, len(list_intersected_faces)):
        ...
        if (
            0 < vertices[faces[list_intersected_faces[i][0]][0]][2] < 80
            and 0 < vertices[faces[list_intersected_faces[i][1]][0]][2] < 80
        ):
...
def main():
    ...
    dsets = hdf5.load_datasets(
        "data/with_collisions.hf5", data_names=["vert", "edge", "face"]
    )
